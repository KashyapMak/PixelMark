// Client-side MD5 algorithm for cryptographic checksum (matches ShareX Hash tool)
...
export function computeMd5FromBytes(bytes: Uint8Array): string {
  ...
  for (let j = 0; j < remaining; j++) {
    tail[j] = bytes[n - remaining + j];
  }
  tail[remaining] = 0x80;
  if (remaining >= 56) {
    const chunk: number[] = [];
    for (let j = 0; j < 16; j++) {
      chunk[j] = tail[j * 4] | (tail[j * 4 + 1] << 8) | (tail[j * 4 + 2] << 16) | (tail[j * 4 + 3] << 24);
    }
    md5Cycle(state, chunk);
    tail.fill(0);
  }
  ...
  for (let j = 0; j < 4; j++) {
    const val = state[j];
    for (let b = 0; b < 4; b++) {
      hex += ((val >>> (b * 8)) & 0xff).toString(16).padStart(2, '0');
    }
  }
  return hex;
}
...
export function getAspectRatioString(width: number, height: number): string {
  ...
  // Standard ratio checks with tolerance
